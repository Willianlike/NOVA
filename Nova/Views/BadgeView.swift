...
extension UIView {
    ...
    public func badge(text badgeText: String?, badgeEdgeInsets: UIEdgeInsets?, appearance: BadgeAppearance) {
        ...
        let centerY = appearance.distanceFromCenterY == 0 && !appearance.inCenter ? -(bounds.size.height / 2) : appearance.distanceFromCenterY
        
        //The distance from the center of the view (horizontally)
        let centerX = appearance.distanceFromCenterX == 0 && !appearance.inCenter ? (bounds.size.width / 2) : appearance.distanceFromCenterX
